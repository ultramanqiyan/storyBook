-- 正确的书籍清理迁移
-- 执行时间: 2026-03-17
-- 目标: 英文版31本（23本AI英文 + 8本003/004英文），中文版8本（8本003/004中文）
...
-- 2. 003/004系列中英文 (16本): 将从线上同步

-- 1. 删除谜题（依赖章节）
DELETE FROM puzzles WHERE chapter_id IN (
  SELECT chapter_id FROM chapters 
  WHERE book_id LIKE 'preset-ai-%-zh'
     OR book_id = 'preset-adventure-001'
     OR book_id = 'preset-adventure-002'
     OR book_id = 'preset-fantasy-001'
     OR book_id = 'preset-fantasy-002'
     OR book_id = 'preset-romance-001'
     OR book_id = 'preset-romance-002'
     OR book_id = 'preset-business-001'
     OR book_id = 'preset-business-002'
     OR book_id = 'preset-adventure-001-en'
     OR book_id = 'preset-adventure-002-en'
     OR book_id = 'preset-fantasy-001-en'
     OR book_id = 'preset-fantasy-002-en'
     OR book_id = 'preset-romance-001-en'
     OR book_id = 'preset-romance-002-en'
     OR book_id = 'preset-business-001-en'
     OR book_id = 'preset-business-002-en'
);
