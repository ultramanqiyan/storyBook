...
const cardNameMapping = {
  en: {
    adventure: {
      ...
      adventure: {
        'Explore': 'Exploration',
        'Solve Puzzles': 'Puzzle Solving',
        'Guardian Mission': 'Rescue',
        'Research': 'Discover Secrets',
        'Observe Animals': 'Animal Watching'
      },
      equipment: {
        'Old Map': 'Map',
        'Magic Seed': 'Planting',
    ...
    fantasy: {
      weather: {
        'Fire Storm': 'Magic Storm'
      },
      terrain: {
        'Dragon Nest': "Dragon's Lair",
        'Dragon Academy': 'Magic Tower',
        'Locked Room': 'Cavern'
      },
      adventure: {},
      equipment: {
        'Seal Scroll': 'Sealing Scroll'
      }
    },
    romance: {
      weather: {
        'Rainy': 'Rainy Day',
        'Sunny': 'Sunny Day',
        'Cherry Blossoms': 'Cherry Blossom Rain'
      },
      terrain: {
        'Studio': 'Gallery',
        'Street': 'Old Street'
...
for (const bookId of dbBooks) {
  ...
  try {
    ...
    const nameMapping = cardNameMapping[lang]?.[bookType];
    ...
    for (const card of plotCards) {
      ...
      if (nameMapping && nameMapping[subType] && nameMapping[subType][cardName]) {
        const standardName = nameMapping[subType][cardName];
        const standardCard = standardConfig?.[subType]?.find(c => c.name === standardName);
        ...
        if (standardCard) {
          allUpdates.push({
            ...
            oldIcon: currentIcon,
            newIcon: standardCard.icon
          });
        }
      } else if (standardConfig && standardConfig[subType]) {
        const standardCard = standardConfig[subType].find(c => c.name === cardName);
        
        if (standardCard && currentIcon !== standardCard.icon) {
          allUpdates.push({
            bookId,
            cardId: card.card_id,
            subType,
...
console.log(`\n=== 卡牌更新统计 ===`);
console.log(`需要更新的卡牌: ${allUpdates.length} 张`);

const nameChanges = allUpdates.filter(u => u.oldName !== u.newName);
const iconOnlyChanges = allUpdates.filter(u => u.oldName === u.newName);

console.log(`需要改名: ${nameChanges.length} 张`);
console.log(`只需更新icon: ${iconOnlyChanges.length} 张`);

const sqlStatements = [];

for (const update of allUpdates) {
  if (update.oldName !== update.newName) {
    sqlStatements.push(`-- ${update.bookId}: ${update.oldName} -> ${update.newName}`);
    sqlStatements.push(`UPDATE plot_cards SET name = '${update.newName.replace(/'/g, "''")}', icon = '${update.newIcon}' WHERE card_id = '${update.cardId}';`);
  } else {
    sqlStatements.push(`-- ${update.bookId}: ${update.oldName} icon update`);
    sqlStatements.push(`UPDATE plot_cards SET icon = '${update.newIcon}' WHERE card_id = '${update.cardId}';`);
  }
}

const migrationFile = path.join(MIGRATIONS_DIR, '0711_fix_preset_card_names_and_icons.sql');
const content = `-- 修复预设书籍卡牌名称和emoji
-- 执行时间: ${new Date().toISOString()}
-- 总计: ${allUpdates.length} 条更新语句
...
console.log(`\n迁移脚本已生成: ${migrationFile}`);

const jsonOutput = path.join(__dirname, 'card-updates.json');
fs.writeFileSync(jsonOutput, JSON.stringify(allUpdates, null, 2));
console.log(`更新详情已保存: ${jsonOutput}`);
