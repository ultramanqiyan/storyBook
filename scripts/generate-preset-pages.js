          ...
          <div class="book-info-header">
            ...
            <div class="book-meta-info">
              ...
              <div class="action-buttons">
                ${chapters.length > 0 ? `<a href="../chapter.html?id=${chapters[0].chapterId}&is_preset=1" class="action-btn">${isZh ? '开始阅读' : 'Start Reading'}</a>` : ''}
          ...
          <div id="leftPageContent">
            <div class="chapter-toc">
              ${leftChapters.sort((a, b) => a.orderNum - b.orderNum).map((ch, i) => `
                <a href="../chapter.html?id=${ch.chapterId}&is_preset=1" class="chapter-toc-item" style="animation: fadeIn 0.5s ease-out ${i * 0.1}s backwards;">
          ...
          <div id="rightPageContent">
            <div class="chapter-toc">
              ${rightChapters.sort((a, b) => a.orderNum - b.orderNum).map((ch, i) => `
                <a href="../chapter.html?id=${ch.chapterId}&is_preset=1" class="chapter-toc-item" style="animation: fadeIn 0.5s ease-out ${(i + halfChapters) * 0.1}s backwards;">
                  <span class="chapter-number">${isZh ? '第' : 'Ch. '}${romanNumerals[ch.orderNum - 1] || ch.orderNum}</span>
                  <span class="chapter-dots"></span>
                  <span class="chapter-title">${ch.title}</span>
                </a>
  ...
  <script>
    ...
    function renderChapters() {
      ...
      rightContent.innerHTML = '<div class="chapter-toc">' + chapters.slice(half).map((ch, i) => 
        '<a href="../chapter.html?id=' + ch.chapterId + '&is_preset=1" class="chapter-toc-item" style="animation: fadeIn 0.5s ease-out ' + ((i + half) * 0.1) + 's backwards;">' +
          '<span class="chapter-number">' + (isZh ? '第' : 'Ch. ') + (romanNumerals[ch.orderNum - 1] || ch.orderNum) + '</span>' +
          '<span class="chapter-dots"></span>' +
          '<span class="chapter-title">' + ch.title + '</span>' +
        '</a>'
...
function main() {
  ...
  books.forEach(book => {
    ...
    console.log(`✓ 生成: ${filename} (${bookCharacters.length} 角色, ${bookChapters.length} 章节)`);
    generatedCount++;
  });
  
  console.log(`\n完成！共生成 ${generatedCount} 个静态页面`);
