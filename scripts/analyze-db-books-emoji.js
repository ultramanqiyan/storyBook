...
console.log(`\n${'='.repeat(70)}`);
console.log(`数据库预设书籍Emoji分析报告 (仅数据库中的39本书)`);
...
console.log(`AI系列书籍 (英文): ${aiBooks.length}`);
console.log(`非AI英文书籍: ${enBooks.length}`);
...
console.log(`\n=== 卡牌匹配统计 ===`);
console.log(`AI系列缺失卡牌: ${aiMissingCards.length} (无标准配置)`);
console.log(`非AI书籍缺失卡牌: ${otherMissingCards.length}`);

console.log(`\n\n${'='.repeat(70)}`);
console.log(`AI系列书籍缺失卡牌详情 (${aiMissingCards.length}张)`);
console.log(`${'='.repeat(70)}\n`);

const aiCardsBySubType = {};
for (const card of aiMissingCards) {
  if (!aiCardsBySubType[card.subType]) aiCardsBySubType[card.subType] = new Set();
  aiCardsBySubType[card.subType].add(card.cardName);
}

for (const [subType, cards] of Object.entries(aiCardsBySubType)) {
  console.log(`[${subType}] (${cards.size}种):`);
  const cardList = [...cards];
  console.log(`  ${cardList.join(', ')}`);
}

console.log(`\n\n${'='.repeat(70)}`);
console.log(`非AI书籍缺失卡牌详情 (${otherMissingCards.length}张)`);
console.log(`${'='.repeat(70)}\n`);

const otherCardsByType = {};
for (const card of otherMissingCards) {
  const key = `${card.bookType}|${card.subType}`;
  if (!otherCardsByType[key]) otherCardsByType[key] = new Set();
  otherCardsByType[key].add(`${card.cardName}(${card.lang})`);
}

for (const [key, cards] of Object.entries(otherCardsByType)) {
  console.log(`[${key}] (${cards.size}种):`);
  const cardList = [...cards];
  console.log(`  ${cardList.join(', ')}`);
}
...
console.log(`   - 需要新增AI类型的标准emoji配置`);
console.log(`   - 缺失卡牌: ${aiMissingCards.length}张`);
console.log(`   - 建议为AI系列创建独立的emoji配置`);
...
const jsonOutput = path.join(__dirname, 'db-books-emoji-analysis.json');
fs.writeFileSync(jsonOutput, JSON.stringify({
  ...
  aiMissingCards: aiMissingCards.map(c => ({
    bookId: c.bookId,
    subType: c.subType,
    cardName: c.cardName,
    currentIcon: c.currentIcon
  })),
  otherMissingCards: otherMissingCards.map(c => ({
    bookId: c.bookId,
    bookType: c.bookType,
    subType: c.subType,
    cardName: c.cardName,
