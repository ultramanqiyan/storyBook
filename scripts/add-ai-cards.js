...
const zhData = JSON.parse(fs.readFileSync('config/plot-options.json', 'utf8'));
...
// Merge new AI cards into existing data
function mergeCards(existingData, newCards) {
  for (const bookType of Object.keys(newCards)) {
    for (const subType of Object.keys(newCards[bookType])) {
      if (!existingData[bookType]) {
        existingData[bookType] = {};
      }
      if (!existingData[bookType][subType]) {
        existingData[bookType][subType] = [];
      }
      // Add new cards to the end of existing cards
      existingData[bookType][subType] = [
        ...existingData[bookType][subType],
        ...newCards[bookType][subType]
      ];
    }
  }
  return existingData;
...
// Merge and save
const updatedZH = mergeCards(JSON.parse(JSON.stringify(zhData)), newAICardsZH);
const updatedEN = mergeCards(JSON.parse(JSON.stringify(enData)), newAICardsEN);

// Count total cards
let totalZH = 0;
let totalEN = 0;
for (const bookType of Object.keys(updatedZH)) {
  for (const subType of Object.keys(updatedZH[bookType])) {
    totalZH += updatedZH[bookType][subType].length;
    totalEN += updatedEN[bookType][subType].length;
  }
}

console.log('Updated Chinese cards:', totalZH);
console.log('Updated English cards:', totalEN);

// Save updated files
fs.writeFileSync('config/plot-options.json', JSON.stringify(updatedZH, null, 2));
fs.writeFileSync('config/zh/plot-options.json', JSON.stringify(updatedZH, null, 2));
fs.writeFileSync('config/en/plot-options.json', JSON.stringify(updatedEN, null, 2));

console.log('\nFiles updated successfully!');
console.log('- config/plot-options.json');
console.log('- config/zh/plot-options.json');
console.log('- config/en/plot-options.json');
