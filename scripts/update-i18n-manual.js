...
// Function to format cards for insertion
function formatCardsForJS(cards, indent) {
  return cards.map(card => {
    return `${indent}{ name: "${card.name}", icon: "${card.icon}", description: "${card.description}" }`;
  }).join(',\n');
}

// For each book type and sub type, add new cards
...
for (const bookType of bookTypes) {
  for (const subType of subTypes) {
    const newCards = newAICardsEN[bookType][subType];
    const cardsStr = formatCardsForJS(newCards, '          ');
    
    // Find the pattern for the last element in the array
    // Pattern: { name: "...", icon: "...", description: "..." }\n        ]\n      },
    // We need to add the new cards before the closing bracket
    
    // Create a regex to find the end of each subType array in English section
    const pattern = new RegExp(
      `(// English section[\\s\\S]*?${bookType}:\\s*\\{[\\s\\S]*?${subType}:\\s*\\[[\\s\\S]*?\\{ name: "[^"]+", icon: "[^"]+", description: "[^"]+" \\}\\s*)\\]`,
      'g'
    );
...
// Simpler approach: find specific markers and insert
// We'll look for the last card in each section and add after it

// For English adventure/weather, find the last card and add new cards
const lastCardPatterns = {
  'en': {
    'adventure': {
      'weather': 'Solar Eclipse',
      'terrain': 'Rainforest',
      'adventure': 'Survival',
      'equipment': 'Astronomical Telescope'
    },
    'fantasy': {
      'weather': 'Genesis Dawn',
      'terrain': 'Mystic Sea',
      'adventure': 'Guardian Mission',
      'equipment': 'Dragon Heart'
    },
    'romance': {
      'weather': 'Rainbow',
      'terrain': 'Gallery',
      'adventure': 'Getting Back Together',
      'equipment': 'Canvas'
    },
    'business': {
      'weather': 'Clearing Up',
...
// Now let's insert the new cards after each last card
for (const lang of ['en', 'zh']) {
  const newCards = lang === 'zh' ? newAICardsZH : newAICardsEN;
  
  for (const bookType of bookTypes) {
    for (const subType of subTypes) {
      const lastCardName = lastCardPatterns[lang][bookType][subType];
      const cardsToAdd = newCards[bookType][subType];
      const cardsStr = formatCardsForJS(cardsToAdd, '          ');
      
      // Find the last card and add new cards after it
      // The pattern is: { name: "LastCardName", icon: "...", description: "..." }
      // We need to add a comma and the new cards after this
      
      const escapedName = lastCardName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      const pattern = new RegExp(
        `(\\{ name: "${escapedName}", icon: "[^"]+", description: "[^"]+" \\})`,
        'g'
      );
      ...
      if (pattern.test(content)) {
        ...
        if (matches && matches.length > 0) {
          // We need to find the specific match for this book type and sub type
...
// Alternative approach: manually construct the replacement
// Since the file structure is predictable, we can find specific sections and update them

console.log('The file structure is complex. Using manual section replacement...');

// Let's try a different approach - find the section by book type and sub type
// and replace the entire array

// For now, let's just output the cards that need to be added
console.log('\n=== New AI Cards to Add ===\n');
console.log('English version cards:');
console.log(JSON.stringify(newAICardsEN, null, 2));
console.log('\nChinese version cards:');
console.log(JSON.stringify(newAICardsZH, null, 2));

console.log('\n\nPlease manually add these cards to functions/_shared/i18n-config.js');
console.log('The file needs to be updated in the plotOptions section for both en and zh.');
