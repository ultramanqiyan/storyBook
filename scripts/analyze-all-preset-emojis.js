...
const results = [];
const allMissingCards = [];
const allCardsInBooks = new Set();
...
for (const bookId of presetBooks) {
  ...
  try {
    ...
    const iconCounts = {};
    const standardConfig = standardEmojis[lang]?.[bookType];
    ...
    for (const card of plotCards) {
      ...
      const cardKey = `${bookType}|${subType}|${cardName}`;
      allCardsInBooks.add(cardKey);
      ...
      if (standardConfig && standardConfig[subType]) {
        const standardCard = standardConfig[subType].find(c => c.name === cardName);
        ...
        if (standardCard) {
          ...
          analysis.unmatchedCards.push({
            ...
            name: cardName,
            currentIcon,
            reason: 'Card name not in standard config'
          });
...
console.log(`=== 书籍统计 ===`);
console.log(`总书籍数: ${results.length}`);
console.log(`AI系列书籍: ${aiBooks.length} (无标准emoji配置)`);
console.log(`其他英文书籍: ${enBooks.length}`);
console.log(`中文书籍: ${zhBooks.length}`);

const booksWithDuplicates = results.filter(r => r.duplicateEmojis.length > 0);
console.log(`\n=== 问题统计 ===`);
console.log(`有重复emoji的书籍: ${booksWithDuplicates.length}`);

const missingByType = {};
for (const card of allMissingCards) {
  const key = `${card.bookType}|${card.subType}`;
  if (!missingByType[key]) missingByType[key] = { type: card.bookType, subType: card.subType, cards: [] };
  missingByType[key].cards.push({ name: card.cardName, lang: card.lang, bookId: card.bookId });
}

console.log(`\n${'='.repeat(60)}`);
console.log(`标准配置中缺失的卡牌 (按类型分组)`);
console.log(`${'='.repeat(60)}\n`);

const aiMissingCards = allMissingCards.filter(c => c.bookType === 'ai');
const otherMissingCards = allMissingCards.filter(c => c.bookType !== 'ai');

console.log(`=== AI系列书籍缺失卡牌: ${aiMissingCards.length} 张 ===`);
console.log(`(AI系列没有标准emoji配置，所有卡牌都算缺失)\n`);

const aiCardsBySubType = {};
for (const card of aiMissingCards) {
  if (!aiCardsBySubType[card.subType]) aiCardsBySubType[card.subType] = [];
  aiCardsBySubType[card.subType].push(card.cardName);
}

for (const [subType, cards] of Object.entries(aiCardsBySubType)) {
  const uniqueCards = [...new Set(cards)];
  console.log(`[${subType}] (${uniqueCards.length}种):`);
  console.log(`  ${uniqueCards.slice(0, 10).join(', ')}${uniqueCards.length > 10 ? '...' : ''}`);
}

console.log(`\n\n=== 非AI书籍缺失卡牌: ${otherMissingCards.length} 张 ===`);
console.log(`(这些卡牌名称在标准配置中找不到)\n`);

const otherCardsByType = {};
for (const card of otherMissingCards) {
  const key = `${card.bookType}|${card.subType}`;
  if (!otherCardsByType[key]) otherCardsByType[key] = { type: card.bookType, subType: card.subType, cards: [] };
  otherCardsByType[key].cards.push(card);
}

for (const [key, data] of Object.entries(otherCardsByType)) {
  const uniqueCards = [...new Set(data.cards.map(c => `${c.cardName}(${c.lang})`))];
  console.log(`[${data.type}|${data.subType}] (${uniqueCards.length}种):`);
  console.log(`  ${uniqueCards.slice(0, 5).join(', ')}${uniqueCards.length > 5 ? '...' : ''}`);
}
...
console.log(`1. AI系列书籍 (${aiBooks.length}本):`);
console.log(`   - 问题: 没有标准emoji配置`);
console.log(`   - 建议: 需要为AI系列创建标准emoji配置，或使用通用emoji`);
console.log(`   - 缺失卡牌数: ${aiMissingCards.length}`);

console.log(`\n2. 非AI英文书籍 (${enBooks.length}本):`);
const enMatched = enBooks.reduce((sum, b) => sum + b.matchedCards.length, 0);
const enUnmatched = enBooks.reduce((sum, b) => sum + b.unmatchedCards.length, 0);
console.log(`   - 匹配标准: ${enMatched} 张`);
console.log(`   - 不匹配标准: ${enUnmatched} 张`);

console.log(`\n3. 中文书籍 (${zhBooks.length}本):`);
const zhMatched = zhBooks.reduce((sum, b) => sum + b.matchedCards.length, 0);
const zhUnmatched = zhBooks.reduce((sum, b) => sum + b.unmatchedCards.length, 0);
console.log(`   - 匹配标准: ${zhMatched} 张`);
console.log(`   - 不匹配标准: ${zhUnmatched} 张`);

const jsonOutput = path.join(__dirname, 'all-preset-emoji-analysis.json');
fs.writeFileSync(jsonOutput, JSON.stringify({
  summary: {
    ...
    aiMissingCards: aiMissingCards.length,
    otherMissingCards: otherMissingCards.length
  },
  missingCards: allMissingCards,
  books: results
}, null, 2));
console.log(`\n详细分析已保存到: ${jsonOutput}`);
