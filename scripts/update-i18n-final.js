...
// Last card names for each section (to find insertion points)
const lastCards = {
  en: {
    adventure: { weather: 'Solar Eclipse', terrain: 'Rainforest', adventure: 'Survival', equipment: 'Astronomical Telescope' },
    fantasy: { weather: 'Genesis Dawn', terrain: 'Mystic Sea', adventure: 'Guardian Mission', equipment: 'Dragon Heart' },
    romance: { weather: 'Rainbow', terrain: 'Gallery', adventure: 'Getting Back Together', equipment: 'Canvas' },
    business: { weather: 'Clearing Up', terrain: 'Coffee Shop', adventure: 'Rise', equipment: 'Directory' }
  },
  zh: {
    adventure: { weather: '日食', terrain: '雨林', adventure: '生存', equipment: '天文望远镜' },
    fantasy: { weather: '创世曙光', terrain: '神秘之海', adventure: '守护使命', equipment: '龙之心' },
    romance: { weather: '彩虹', terrain: '画廊', adventure: '复合', equipment: '画布' },
    business: { weather: '转晴', terrain: '咖啡厅', adventure: '崛起', equipment: '通讯录' }
  }
};

// Format cards for insertion
function formatCardsForJS(cards, indent) {
  return cards.map(card => {
    return `,\n${indent}{ name: "${card.name}", icon: "${card.icon}", description: "${card.description}" }`;
  }).join('');
}

// Insert new cards after the last card in each section
for (const lang of ['en', 'zh']) {
  const newCards = lang === 'zh' ? newAICardsZH : newAICardsEN;
  
  for (const bookType of ['adventure', 'fantasy', 'romance', 'business']) {
    for (const subType of ['weather', 'terrain', 'adventure', 'equipment']) {
      const lastCardName = lastCards[lang][bookType][subType];
      const cardsToAdd = newCards[bookType][subType];
      const cardsStr = formatCardsForJS(cardsToAdd, '          ');
      
      // Escape special characters in the card name for regex
      const escapedName = lastCardName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      
      // Create pattern to match the last card
      // Pattern: { name: "LastCardName", icon: "...", description: "..." }
      const pattern = new RegExp(
        `(\\{ name: "${escapedName}", icon: "[^"]+", description: "[^"]+" \\})(\\s*\\])`,
        'g'
      );
      
      // Replace: add new cards before the closing bracket
      content = content.replace(pattern, `$1${cardsStr}$2`);
    }
  }
}
...
console.log('Successfully updated functions/_shared/i18n-config.js');
console.log('Added 128 AI cards (64 Chinese + 64 English)');

// Verify the update
const updatedContent = fs.readFileSync(configPath, 'utf8');
const enMatch = updatedContent.match(/en:\s*\{[\s\S]*?adventure:\s*\{[\s\S]*?weather:\s*\[([\s\S]*?)\]/);
if (enMatch) {
  const weatherCards = (enMatch[1].match(/\{ name:/g) || []).length;
  console.log(`\nVerification: English adventure/weather now has ${weatherCards} cards`);
}
