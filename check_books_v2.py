...
def check_file(filepath):
    ...
    try:
        ...
        return [{'type': 'read_error', 'line': 0, 'content': str(e)}]
    ...
    for line_num, line in enumerate(lines, 1):
        ...
        if '<!--' in line or '-->' in line:
            continue
        # 检测文本中的 -- (前后是字母或空格，表示破折号用法)
        dash_match = re.search(r'(?<=[a-zA-Z\s])--(?=[a-zA-Z\s])', line)
        if dash_match:
            issues.append({
                'type': 'double_dash',
                'line': line_num,
        ...
        for char in line:
            ...
            # 检测其他非英文字符 (如日文、韩文、俄文等，但排除常见外语字母)
            # 允许的法语/西班牙语等常见字母
            allowed_chars = 'àâäéèêëïîôùûüÿçœæñóíáúü¿¡'
...
def generate_report(results, total_files, total_issues, output_file):
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        type_names = {
            ...
            'non_ascii': '非ASCII字符 (非常见外语字母)',
            'read_error': '读取错误 (编码问题)'
        }
...
if __name__ == '__main__':
    ...
    output_file = r'd:\trae_job\storyBook\check_report_v2.txt'
