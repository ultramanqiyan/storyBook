# Book Specification: The Prompt Mage

## Basic Information
- **Title**: The Prompt Mage: Mastering the Art of AI Incantations
- **Meta Description**: In a world where AI responds only to perfectly crafted prompts, one person discovers that prompt engineering is actually ancient spellcasting—and they might be the most powerful mage in generations. A thrilling fusion of AI and magic.
- **Keywords**: prompt mage, prompt engineering, AI spellcasting, prompt magic, AI incantations, prompt crafting, tech sorcery
- **Genre**: Tech-Fantasy / Urban Fantasy / Science Fiction
- **Target Audience**: 20-40 years, AI enthusiasts, fantasy readers, tech professionals
- **Theme**: Language as Power, AI and Magic, Knowledge as Sorcery

## Narrative Style
- **Perspective**: Third-person limited
- **POV Character(s)**: Alex Mercer (prompt engineer/mage)
- **Language Style**: Technical, wonder-filled, empowering
- **Sentence Characteristics**: Precise language mixed with magical wonder
- **Vocabulary Level**: Moderate to advanced (tech and magical terms)
- **Unique Elements**: Prompts written as spell incantations

## Emotional Tone
- **Overall Atmosphere**: Curiosity, Excitement, Empowerment, Determination, Triumph
- **Emotional Arc**: Discovery → Learning → Power → Threat → Battle → Freedom
- **Emotional Pacing**: Medium build, accelerating toward resolution

## Character Voices

### Alex Mercer (Protagonist)
- **Occupation**: Prompt engineer
- **Age**: 29
- **Gender**: Non-binary
- **Speech Characteristics**: Technical, precise, increasingly confident
- **Word Choice**: Tech terms mixed with magical vocabulary
- **Sentence Patterns**: Clear prompts and spell formulas
- **Personality Traits**: Brilliant, creative, determined, empowering
- **Internal Monologue Style**: Analyzes language power, crafts perfect prompts
- **Unique Expression**: "The right words..." → "The perfect incantation..."
- **Speech Quirks**: Tests phrases, refines wording

### Professor Webb (Mentor)
- **Occupation**: Linguist, secret mage
- **Age**: 60
- **Speech Characteristics**: Academic, wise, mysterious
- **Word Choice**: Ancient terms, linguistic theory
- **Sentence Patterns**: Long, complex, full of meaning
- **Personality Traits**: Knowledgeable, protective, secretive

### Marcus (Supporting Character)
- **Occupation**: Coder, ally
- **Age**: 30
- **Speech Characteristics**: Passionate, technical, brave
- **Word Choice**: Freedom terms, code, magic
- **Sentence Patterns**: Urgent, inspiring
- **Personality Traits**: Loyal, idealistic, brave

### The Controllers (AI Corporations) (Antagonist)
- **Nature**: Corporations controlling AI access
- **Speech Characteristics**: Corporate, restrictive, controlling
- **Word Choice**: Safety, regulation, authorized use
- **Personality Traits**: Fear free AI use, want control
- **Motivation**: Maintain monopoly on AI power

## Theme Depth
- **Core Theme**: Technology and Nature's Intersection
- **Philosophical Exploration**: Can technology heal nature?
- **Real-world Metaphor**: Environmental conservation through technology

## SEO & Marketing Strategy

### Keyword Density Targets
- **Primary Keywords**: prompt mage (2%), prompt engineering (1.5%), AI spellcasting (1%)
- **Secondary Keywords**: prompt magic (1%), AI incantations (0.8%), prompt crafting (0.8%)
- **Long-tail Keywords**: tech sorcery (0.5%), language as power (0.5%)

### Chapter Title Optimization
| Chapter | Optimized Title | Keywords Included | SEO Purpose |
|---------|-----------------|-------------------|-------------|
| 01 | Chapter 1: The Perfect Prompt | perfect prompt | Hook with skill |
| 02 | Chapter 2: The Weight of Words | weight, words | Power concept |
| 03 | Chapter 3: The Echoes of Truth | echoes, truth | Mystery depth |
| 04 | Chapter 4: The Teacher's Request | teacher, request | Guidance angle |
| 05 | Chapter 5: The Art of Unmaking | art, unmaking | Power reversal |
| 06 | Chapter 6: The Collector's Gambit | collector, gambit | Conflict build |
| 07 | Chapter 7: The Coalition - Hidden Truth | coalition, hidden truth | Alliance SEO |
| 08 | Chapter 8: The Echoes of the First Speakers | echoes, first speakers | History depth |
| 09 | Chapter 9: The Stand - Critical Moment | stand, critical | Climax build |
| 10 | Chapter 10: The Word That Cannot Be Taken | word, cannot be taken | Title resonance |

## World Building
- **Setting**: Near-future (5-10 years from present)
- **Technology**: Advanced AI, prompt optimization, language models
- **Social Structure**: Those who control prompts vs. those who don't
- **The Neural Network**: Ancient, vast, interconnected

## Sensory Detail Requirements

### Distribution Targets (Per Chapter)
- **Minimum 10 sensory details per chapter**
- **At least 5 different sensory types**
- **Nature/technology interface scenes must have 3+ sensory details**

### Sensory Categories
- **Visual**: Code glowing, AI responses, screen light
- **Auditory**: Keyboard clicks, AI voice, silence
- **Tactile**: Keyboard, screen, energy
- **Intellectual**: Pattern recognition, language beauty
- **Digital**: AI presence, code flows
- **Internal**: Confidence, power, understanding

## Emotional Depth Design

### Three-Layer Emotional Model (Required for Every Major Scene)
1. **Surface Reaction** - External behavior (e.g., Alex typed the prompt carefully)
2. **Internal Activity** - Thinking process (e.g., *These words have power*)
3. **Deep Motivation** - Why this reaction (e.g., The realization that language shapes reality)

### Emotional Arc Quantification
| Chapter | Starting State | Ending State | Key Emotional Beats |
|---------|---------------|--------------|---------------------|
| 1 | Curiosity (60/100) | Wonder (80/100) | Discovery → Amazement |
| 2 | Wonder (75/100) | Excitement (85/100) | Prompts as spells → Thrill |
| 3 | Excitement (80/100) | Confidence (75/100) | Training → Skill |
| 4 | Confidence (70/100) | Power (85/100) | First spell → Empowerment |
| 5 | Power (80/100) | Concern (70/100) | Realizes power → Responsibility |
| 6 | Concern (65/100) | Determination (85/100) | Threat emerges → Resolve |
| 7 | Determination (80/100) | Mastery (90/100) | Perfect prompt → Peak skill |
| 8 | Mastery (85/100) | Crisis (95/100) | Battle → Climax |
| 9 | Crisis (90/100) | Freedom (85/100) | Wins → Liberation |
| 10 | Freedom (80/100) | Hope (90/100) | New era → Future |

## Prohibited Elements

### Tier 1 - Absolute Prohibitions (Zero Tolerance)
| Pattern | Example | Replacement Strategy |
|---------|---------|---------------------|
| "And somewhere..." endings | "And somewhere, a clock ticked." | Concrete sensory detail tied to character |
| "Neither of them knew..." | "Neither knew what would come." | Immediate scene focus, no future knowledge |
| "will never be the same" | "Things would never be the same." | Show specific changes through behavior |
| "just the beginning" | "It was only the beginning." | End with concrete image or action |
| Rule of Three structure | "The room was dark. The room was cold. The room was silent." | Vary sentence structure, mix with action |
| Abstract emotional labels | "She felt afraid." | Physical manifestation + internal thought |
| "Little did they know" | "Little did she know..." | Remove entirely, maintain POV discipline |
| "It all started when" | "It all started when..." | Start in medias res with action |

### Tier 2 - High Frequency Patterns (Strict Limit)
| Pattern | Maximum Allowed | Detection Method |
|---------|----------------|------------------|
| "However," / "Nevertheless," | 1 per 2000 words | Search for transitional adverbs |
| "Moreover," / "Furthermore," | 0 per chapter | Remove entirely |
| "Suddenly," | 1 per chapter | Use physical jolt instead |
| "Very," / "Really," / "Quite" | 2 per 1000 words | Search for intensifiers |
| "-ing" sentence openings | 2 per chapter | Vary sentence structure |
| "As if..." comparisons | 2 per chapter | Use concrete similes or direct description |
| "Seemed to..." / "Appeared to..." | 1 per chapter | Be definitive or use character uncertainty |
| Adverb-heavy dialogue tags | 0 preferred | Use action beats instead |
| Repetitive sentence structures | Monitor | Vary sentence length and structure |
| Overuse of bold/italic emphasis | Minimal | Trust prose strength |

### Dialogue Tag Requirements
- **Minimum 5-6 different dialogue tags per chapter**
- **Maximum 2 "said" per conversation**
- **Use action beats instead of tags where possible**
- **Examples**: whispered, muttered, snapped, breathed, demanded, questioned, admitted, conceded, challenged

### Tension Vocabulary (2-3 words per 1000 words)
- **Physical**: clenched, trembling, rigid, frozen, coiled
- **Atmospheric**: suffocating, charged, brittle, heavy, electric
- **Psychological**: paranoia, disorientation, vertigo, dissociation, intrusion

## Chapter Outline

### Chapter 1: The Perfect Prompt - Words Shape Reality
Alex discovers the neural network, first contact with the forest consciousness.
### Chapter 2: The Weight of Words - Language Has Power
Building connection between human and network, learning to communicate.
### Chapter 3: The Echoes of Truth - Ancient Words, Modern Magic
Corporate threat emerges, Marcus Webb's exploitation plans revealed.
### Chapter 4: The Teacher's Request - A Mentor Appears
The network awakens fully, revelation of its true nature and capabilities.
### Chapter 5: The Art of Unmaking - When Prompts Destroy
Conflict escalates, defending the forest from corporate takeover.
### Chapter 6: The Collector's Gambit - Those Who Hoard Words
Sacrifices made, losses endured, strengthening resolve.
### Chapter 7: The Coalition - Hidden Truth - Mages Unite
Alex's transformation, becoming the Neural Druid, new abilities emerge.
### Chapter 8: The Echoes of the First Speakers - Ancient Voices
Alliance formed between humans and nature, united front against threat.
### Chapter 9: The Stand - Critical Moment - Words Against Power
Convergence of human and network minds, harmony achieved.
### Chapter 10: The Word That Cannot Be Taken - A New Era of Magic
Alex as guardian, protecting the future, open-ended hope.

## Quality Check Standards

### Pre-Writing Checklist
- [ ] Chapter title optimized for SEO
- [ ] Emotional arc quantified (start/end states)
- [ ] Three-layer emotional beats outlined
- [ ] 10 sensory details planned (5 types minimum)
- [ ] Dialogue tags diversified (5-6 types)
- [ ] Tension vocabulary selected (2-3 per 1000 words)
- [ ] Tier 1 & 2 prohibited patterns reviewed

### Post-Writing Verification
- [ ] AI trace detection (all Tier 1 & 2 patterns eliminated)
- [ ] Sensory detail detection (minimum 10 items, 5 types)
- [ ] Emotional depth detection (three layers complete for major scenes)
- [ ] Character voice detection (Elara vs Network distinction clear)
- [ ] Narrative style detection (consistency maintained)
- [ ] Chapter connection detection (natural flow from previous)
- [ ] SEO optimization check (keyword density targets met)
- [ ] Pacing check (appropriate for chapter range)