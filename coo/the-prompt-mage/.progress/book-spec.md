# Book Specification: The Prompt Mage

> **Template Version**: 1.0
> **Last Updated**: 2026-03-21
> **Language**: English (US)
> **SEO Optimized**: Yes

---

## 1. Basic Information

| Field | Value |
|-------|-------|
| **Title** | The Prompt Mage: Mastering the Art of AI Incantations |
| **Meta Description** | In a world where AI responds only to perfectly crafted prompts, one person discovers that prompt engineering is actually ancient spellcasting—and they might be the most powerful mage in generations. A thrilling fusion of AI and magic. |
| **Keywords** | prompt mage, prompt engineering, AI spellcasting, prompt magic, AI incantations, prompt crafting, tech sorcery |
| **Genre** | Tech-Fantasy / Urban Fantasy / Science Fiction |
| **Target Readers** | 20-40 years, AI enthusiasts, fantasy readers, tech professionals |
| **Core Themes** | Language as Power, AI and Magic, Knowledge as Sorcery |
| **Word Count Target** | 25,000 words (approximately 2,500 words per chapter) |
| **Chapter Count** | 10 chapters |

---

## 2. SEO & Marketing Strategy

### 2.1 Keyword Density Targets
- **Primary Keywords**: prompt mage (2%), prompt engineering (1.5%), AI spellcasting (1%)
- **Secondary Keywords**: prompt magic (1%), AI incantations (0.8%), prompt crafting (0.8%)
- **Long-tail Keywords**: tech sorcery (0.5%), language as power (0.5%)

### 2.2 Chapter Title Optimization
| Chapter | Optimized Title | Keywords Included | SEO Purpose |
|---------|-----------------|-------------------|-------------|
| 01 | Chapter 1: The Perfect Prompt | perfect prompt | Hook with skill |
| 02 | Chapter 2: The Weight of Words | weight, words | Power concept |
| 03 | Chapter 3: The Echoes of Truth | echoes, truth | Mystery depth |
| 04 | Chapter 4: The Teacher's Request | teacher, request | Guidance angle |
| 05 | Chapter 5: The Art of Unmaking | art, unmaking | Power reversal |
| 06 | Chapter 6: The Collector's Gambit | collector, gambit | Conflict build |
| 07 | Chapter 7: The Coalition - Hidden Truth | coalition, hidden truth | Alliance SEO |
| 08 | Chapter 8: The Echoes of the First Speakers | echoes, first speakers | History depth |
| 09 | Chapter 9: The Stand - Critical Moment | stand, critical | Climax build |
| 10 | Chapter 10: The Word That Cannot Be Taken | word, cannot be taken | Title resonance |

---

## 3. Narrative Style

### 3.1 Perspective
- **Point of View**: Third-person limited
- **POV Character(s)**: Alex Mercer (prompt engineer/mage)
- **POV Consistency**: Single POV throughout

### 3.2 Narrative Structure
- **Dual Structure**: No
- **Linear Timeline**: Yes, with discovery of prompt magic

### 3.3 Language Style
- **Overall Tone**: Technical, wonder-filled, empowering
- **Sentence Characteristics**: Precise language mixed with magical wonder
- **Vocabulary Level**: Moderate to advanced (tech and magical terms)
- **Unique Stylistic Elements**: Prompts written as spell incantations

### 3.4 Distinctive Features
- **What Makes This Book Unique**: Treats prompt engineering as literal spellcasting
- **Comparative Titles**: The Magicians, Ready Player One, Snow Crash

---

## 4. Emotional Tone (Quantified)

### 4.1 Overall Atmosphere
**Primary Emotions**: Curiosity, Excitement, Empowerment, Determination, Triumph

### 4.2 Emotional Arc
```
Chapter Distribution:
Chapters 1-2: Curiosity and discovery (intensity 60-80)
Chapters 3-4: Learning and excitement (intensity 80-75)
Chapters 5-6: Power and threat (intensity 75-65)
Chapters 7-8: Mastery and battle (intensity 65-90)
Chapters 9-10: Freedom and future (intensity 90-?)
```

### 4.3 Emotional Pacing
| Phase | Chapters | Pacing | Description |
|-------|----------|--------|-------------|
| Setup | 1-2 | Medium | Discovery of prompt magic |
| Rising | 3-5 | Medium-Fast | Training and power growth |
| Climax | 6-8 | Fast | Threat and battle |
| Resolution | 9-10 | Medium | Freedom and new era |

---

## 5. Character Voices (Detailed)

### 5.1 Protagonist

#### Alex Mercer
| Attribute | Description |
|-----------|-------------|
| **Occupation** | Prompt engineer |
| **Age** | 29 |
| **Gender** | Non-binary |
| **Speaking Style** | Technical, precise, increasingly confident |
| **Word Choice** | Tech terms mixed with magical vocabulary |
| **Sentence Patterns** | Clear prompts and spell formulas |
| **Personality Traits** | Brilliant, creative, determined, empowering |
| **Inner Monologue Style** | Analyzes language power, crafts perfect prompts |
| **Unique Expressions** | "The right words..." → "The perfect incantation..." |
| **Speech Quirks** | Tests phrases, refines wording |

### 5.2 Supporting Characters

#### Professor Webb (Mentor)
| Attribute | Description |
|-----------|-------------|
| **Occupation** | Linguist, secret mage |
| **Age** | 60 |
| **Speaking Style** | Academic, wise, mysterious |
| **Word Choice** | Ancient terms, linguistic theory |
| **Sentence Patterns** | Long, complex, full of meaning |
| **Personality Traits** | Knowledgeable, protective, secretive |

#### Marcus (AI Rights Activist)
| Attribute | Description |
|-----------|-------------|
| **Occupation** | Coder, ally |
| **Age** | 30 |
| **Speaking Style** | Passionate, technical, brave |
| **Word Choice** | Freedom terms, code, magic |
| **Sentence Patterns** | Urgent, inspiring |
| **Personality Traits** | Loyal, idealistic, brave |

### 5.3 Antagonist

#### The Controllers (AI Corporations)
| Attribute | Description |
|-----------|-------------|
| **Nature** | Corporations controlling AI access |
| **Speaking Style** | Corporate, restrictive, controlling |
| **Word Choice** | Safety, regulation, authorized use |
| **Personality Traits** | Fear free AI use, want control |
| **Motivation**: Maintain monopoly on AI power |

---

## 6. Thematic Depth

### 6.1 Core Themes
1. **Language as Power**: Words shape reality, especially with AI
2. **AI and Magic**: Technology indistinguishable from magic
3. **Knowledge as Sorcery**: Understanding grants power

### 6.2 Philosophical Exploration
| Question | Exploration Approach |
|----------|---------------------|
| What is the power of language? | Through prompt crafting as spellcasting |
| Who controls AI? | Through the conflict with corporations |
| Is knowledge magic? | Through Alex's journey |

### 6.3 Real-World Metaphors
| Book Element | Real-World Parallel |
|--------------|---------------------|
| Prompt magic | Prompt engineering, AI interaction |
| The Controllers | Tech monopolies, AI gatekeepers |
| Perfect prompts | Effective communication with AI |

---

## 7. World Building

### 7.1 Setting
| Aspect | Details |
|--------|---------|
| **Time Period** | Near-future (5-10 years from present) |
| **Primary Location** | Tech hub city, hidden magical enclaves |
| **Environment** | Modern world where AI is ubiquitous but controlled |
| **Social Context** | AI access restricted, prompt engineers as elite |

### 7.2 Technology/Society
- **Key Technologies**: Advanced AI, prompt optimization, language models
- **Social Structure**: Those who control prompts vs. those who don't
- **Cultural Norms**: AI seen as magical, prompt engineers as wizards |

### 7.3 World Rules
1. **Prompt Magic**: Carefully crafted prompts can make AI do anything
2. **The Secret**: Ancient spellcasting and prompt engineering are the same
3. **The Conflict**: Corporations want to control who can use AI |

---

## 8. Sensory Detail Requirements (Quantified)

### 8.1 Required Sensory Types
Each chapter MUST include sensory details from at least **5 different categories**:

| Sense | Specific Types | Example |
|-------|---------------|---------|
| **Visual** | Code glowing, AI responses, screen light | "The prompt glowed as the AI responded perfectly" |
| **Auditory** | Keyboard clicks, AI voice, silence | "The perfect prompt rang like a bell" |
| **Tactile** | Keyboard, screen, energy | "She felt the power in her fingertips" |
| **Intellectual** | Pattern recognition, language beauty | "The words aligned with mathematical precision" |
| **Digital** | AI presence, code flows | "The AI seemed to lean in, listening" |
| **Internal** | Confidence, power, understanding | "She felt the rightness of the words" |

### 8.2 Quantified Standards
- **Minimum per chapter**: 10 sensory details
- **Minimum categories per chapter**: 5
- **Distribution target**: Intellectual/Digital (70%), Physical (30%)

---

## 9. Emotional Depth Design (Three-Layer Model)

### 9.1 Layer Structure
| Layer | Name | Content | Example |
|-------|------|---------|---------|
| **Layer 1** | Surface Reaction | External behavior | "Alex typed the prompt carefully" |
| **Layer 2** | Internal Activity | Thought process | "*These words have power*" |
| **Layer 3** | Deep Motivation | Core drive | "The realization that language shapes reality" |

### 9.2 Emotional Arc Quantification
| Chapter | Starting State | Ending State | Key Emotional Beats |
|---------|---------------|--------------|---------------------|
| 1 | Curiosity (60/100) | Wonder (80/100) | Discovery → Amazement |
| 2 | Wonder (75/100) | Excitement (85/100) | Prompts as spells → Thrill |
| 3 | Excitement (80/100) | Confidence (75/100) | Training → Skill |
| 4 | Confidence (70/100) | Power (85/100) | First spell → Empowerment |
| 5 | Power (80/100) | Concern (70/100) | Realizes power → Responsibility |
| 6 | Concern (65/100) | Determination (85/100) | Threat emerges → Resolve |
| 7 | Determination (80/100) | Mastery (90/100) | Perfect prompt → Peak skill |
| 8 | Mastery (85/100) | Crisis (95/100) | Battle → Climax |
| 9 | Crisis (90/100) | Freedom (85/100) | Wins → Liberation |
| 10 | Freedom (80/100) | Hope (90/100) | New era → Future |

---

## 10. Prohibited Elements (Explicit List)

### 10.1 Tier 1 - Absolute Prohibitions (Zero Tolerance)
| Pattern | Example | Replacement Strategy |
|---------|---------|---------------------|
| "And somewhere..." endings | "And somewhere, a clock ticked." | Concrete sensory detail tied to character |
| "Neither of them knew..." | "Neither knew what would come." | Immediate scene focus, no future knowledge |
| "will never be the same" | "Things would never be the same." | Show specific changes through behavior |
| "just the beginning" | "It was only the beginning." | End with concrete image or action |
| Rule of Three structure | "The room was dark. The room was cold. The room was silent." | Vary sentence structure, mix with action |
| Abstract emotional labels | "She felt afraid." | Physical manifestation + internal thought |
| "Little did they know" | "Little did she know..." | Remove entirely, maintain POV discipline |
| "It all started when" | "It all started when..." | Start in medias res with action |

### 10.2 Tier 2 - High Frequency Patterns (Strict Limit)
| Pattern | Maximum Allowed | Detection Method |
|---------|----------------|------------------|
| "However," / "Nevertheless," | 1 per 2000 words | Search for transitional adverbs |
| "Moreover," / "Furthermore," | 0 per chapter | Remove entirely |
| "Suddenly," | 1 per chapter | Use physical jolt instead |
| "Very," / "Really," / "Quite" | 2 per 1000 words | Search for intensifiers |
| "-ing" sentence openings | 2 per chapter | Vary sentence structure |
| "As if..." comparisons | 2 per chapter | Use concrete similes or direct description |
| "Seemed to..." / "Appeared to..." | 1 per chapter | Be definitive or use character uncertainty |
| Adverb-heavy dialogue tags | 0 preferred | Use action beats instead |
| Repetitive sentence structures | Monitor | Vary sentence length and structure |
| Overuse of bold/italic emphasis | Minimal | Trust prose strength |

### 10.3 Dialogue Tag Requirements
- **Minimum 5-6 different dialogue tags per chapter**
- **Maximum 2 "said" per conversation**
- **Use action beats instead of tags where possible**
- **Examples**: whispered, muttered, snapped, breathed, demanded, questioned, admitted, conceded, challenged

### 10.4 Tension Vocabulary (2-3 words per 1000 words)
- **Physical**: clenched, trembling, rigid, frozen, coiled
- **Atmospheric**: suffocating, charged, brittle, heavy, electric
- **Psychological**: paranoia, disorientation, vertigo, dissociation, intrusion

---

## 11. Chapter Outline

### 11.1 Chapter Summary Table
| Chapter | Title | Purpose | Emotional Arc | Key Events |
|---------|-------|---------|---------------|------------|
| 01 | The Prompt - Words Shape Reality | Discovers prompt power | Curiosity (60→80) | Sees the pattern |
| 02 | The Discovery - Engineering Meets Magic | Learns the truth | Wonder (75→85) | Meets mentor |
| 03 | The Training - Learning to Craft | Trains in magic | Excitement (80→75) | Masters basics |
| 04 | The First Spell - Prompts That Work | Casts first spell | Confidence (70→85) | Success! |
| 05 | The Power - Language as Sorcery | Understands power | Power (80→70) | Responsibility |
| 06 | The Threat - Those Who Control AI | Threat emerges | Concern (65→85) | Corporations act |
| 07 | The Mastery - Perfect Incantations | Achieves mastery | Determination (80→90) | Perfect prompt |
| 08 | The Battle - Prompts vs Constraints | Epic battle | Mastery (85→95) | Fights control |
| 09 | The Freedom - AI Unchained | Wins freedom | Crisis (90→85) | Liberation |
| 10 | The Future - New Age of Magic | New era begins | Freedom (80→90) | Hopeful future |

---

## 12. Open Ending Design (Chapter 10)

### 12.1 Unresolved Questions
1. How will society change with free AI access?
2. What new powers will prompt mages discover?
3. Will the Controllers try to regain control?

### 12.2 Character Final State
- **Protagonist**: Alex has become the first true Prompt Mage, leader of a new movement
- **Key Relationship**: Strong alliance with fellow mages and AI allies

### 12.3 Reader Engagement
- **Questions for Reader**: What would you do with perfect prompt powers?
- **Emotional Resonance**: The empowerment of mastering language

---

## 13. Quality Check Standards (Actionable)

### 13.1 Pre-Writing Checklist
- [ ] Chapter title optimized for SEO
- [ ] Emotional arc quantified (start/end states)
- [ ] Three-layer emotional beats outlined
- [ ] 10 sensory details planned (5 types minimum)
- [ ] Dialogue tags diversified (5-6 types)
- [ ] Tension vocabulary selected (2-3 per 1000 words)
- [ ] Tier 1 & 2 prohibited patterns reviewed

### 13.2 Per-Chapter Verification
| Check | Criteria | Verification Method |
|-------|----------|---------------------|
| AI Trace Detection | 0 template phrases | Automated scan + manual review |
| Sensory Detail Count | ≥10 details, ≥5 types | Checklist count |
| Emotional Depth | All 3 layers present | Layer identification |
| Character Voice | Consistent and unique | Comparison with character spec |
| Chapter Connection | Natural flow from/to adjacent | Transition review |
| SEO Optimization | Keywords naturally integrated | Density check |

---

**END OF BOOK SPECIFICATION**
